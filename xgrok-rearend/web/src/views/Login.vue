...
function onSubmit() {
  if((ruleFormRef.value as any).checkValidity()){
    ...
    //     })
    //   } else {
    //     showNotification(NotificationTypeEnum.error, '登录失败' || res.message)
    //   }
    // })
  }
}
...
<template>
  <div class="p-12 px-20 h-full flex justify-center items-center overflow-hidden divide-y bg-slate-100 dark:bg-zinc-900">
    <div class="w-400">
      <div class="w-full h-60 rounded-t-xl bg-white dark:bg-zinc-800">
        <img class="mx-auto h-full w-auto" src="../assets/img/icon2.png" alt="xgrok" />
      </div>
      <div class="w-full p-28 rounded-b-xl space-y-12 bg-white text-neutral-600 dark:bg-zinc-950 dark:text-gray-100">
        <h1 class="text-[20px] font-bold text-center">登录你的账号</h1>
        <form ref="ruleFormRef" class="space-y-21 ng-untouched ng-pristine ng-valid" @submit="onSubmit">
          <div class="mb-20">
            <label for="username" class="block text-[14px] text-gray-500 dark:text-gray-400">邮箱</label>
            <input type="email" v-model="form.username" placeholder="" autocomplete="email" required
                   class="w-full px-14 py-5.25 rounded-xl text-[14px]/[21px] border-1 focus:outline-none focus-visible:ring-1
          ...
          <button class="mt-40 block w-full p-10.5 text-center text-white rounded-xl bg-(--el-color-primary) dark:bg-blue-500 cursor-pointer text-[14px]" type="submit">登 录</button>
        </form>
      </div>
    </div>
  </div>
