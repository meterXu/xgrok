...
const emits = defineEmits(['testSpeed'])
...
<template>
  <div class="class-config-item w-full border border-(--border-color) rounded-4xl p-16 flex flex-col justify-between ">
    <div class="flex flex-row items-center justify-between">
      <div class="text-[14px] font-bold h-20 flex items-center justify-between gap-4">
      ...
      <slot name="right-top-icon">
        <img class="w-24 h-24 object-contain" :src="`${cdn}/xgrok-client/${props.serverConfig.operator}.webp`"
             :alt="props.serverConfig.operator"/>
      </slot>
    </div>
    <slot></slot>
    <div class="flex flex-row items-center justify-between">
      ...
      <div class="w-24 text-center">
        <span class="server-status relative inline-block w-12 h-12 rounded-full"
              :class="useStatusClass(serverConfig.is_online)"></span>
      </div>
    </div>
  </div>
</template>
